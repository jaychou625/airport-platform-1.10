...
import com.br.mapper.TaskAccountMapper;
...
public class TrafficTaskStateService {
     ...
     */
    public TaskStateInfo getTaskState(CarInfo carInfo, Boolean mark,String did) {
        //测试用，开始时间戳
        ...
        //获取当前设备号对应的最新执行任务的司机
        jedis.select(5);
        Map<String, String> mapDid = jedis.hgetAll(did);
        if (mapDid.size() == 0) {
            state.setState(-1);
            return state;
        }
        mapDid = SortUtils.sortMapByKey(mapDid);
        Iterator<Map.Entry<String, String>> entriesDid = mapDid.entrySet().iterator();
        Map.Entry<String, String> entryDid = entriesDid.next();
        EquipmentInfo equipmentInfo = JSON.parseObject(entryDid.getValue(), new TypeReference<EquipmentInfo>() {});
        String driverName = equipmentInfo.getAccount().substring(equipmentInfo.getAccount().indexOf("#") + 1);
        driverName = this.taskAccountMapper.selectByAccount(driverName);
        ...
        while (entries.hasNext()){
            ...
            if(taskInfo.getFeedBackName().equals(driverName)){
                flag = true;
                break;
            }
        }
